...
export default function BlogCard (props){
  console.log(props.likes)
    ...
    const updateLike = async(props) => {
      let like_value = props.likes+1
      let user_id = props.id
      const update_like = await fetch(`http://localhost:3001/api/update-like/${like_value}/${user_id}`)
      console.log('final like',update_like)
      const final_like = update_like.json()
      setLike(final_like)
    }
return <>
